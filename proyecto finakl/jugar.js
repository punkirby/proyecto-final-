...
const tutorialSteps = [
    {
        text: 'Bienvenido, aprendiz. Soy el narrador del Concilio Arcano y te guiaré en tu primer turno.',
        target: '#tutorial'
    },
    {
        text: 'Primero, roba una carta con el botón rojo. Cada turno puedes robar una carta.',
        target: '#robarBtn'
    },
    {
    ...
    {
        text: 'Gira una tierra en tu campo para generar maná y poder lanzar criaturas o hechizos.',
        target: '#PlayerBattlefield .cardlands'
    },
    {
        text: 'Selecciona una criatura que no esté tappeada y pulsa ATACAR para declarar el combate.',
        target: '#attackBtn'
    },
    {
        text: 'Cuando el enemigo ataque, puedes bloquear con tus criaturas o usar NO BLOQUEO.',
        target: '#NoBloqueo'
    },
    {
    ...
    {
        text: 'Listo. Cierra este mensaje y comienza tu aventura con estilo de fantasía medieval.',
        target: '#tutorial'
    }
];
...
function applyTutorialSquareMode() {
    ...
    const height = Math.min(width * 0.45, 320);
    const gap = 12;
    tutorialOverlay.style.position = 'absolute';
    tutorialOverlay.style.width = `${width}px`;
    tutorialOverlay.style.height = `${height}px`;
    tutorialOverlay.style.left = `${rect.left}px`;
    tutorialOverlay.style.top = `${rect.bottom - height}px`;
    tutorialOverlay.style.right = 'auto';
    tutorialOverlay.style.bottom = 'auto';
    tutorialOverlay.classList.add('square');
    logElement.style.marginBottom = `${gap}px`;
    logElement.style.height = `calc(100% - ${height + gap}px)`;
}
