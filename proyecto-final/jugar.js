let playerLife = 20;
let enemyLife = 20;
const PLAYER_MAX_LIFE = 20;
const ENEMY_MAX_LIFE = 20;
let turn = 'player';
let playerName = 'Tú';
...
const tutorialSteps = [
    {
        text: 'Bienvenido, aprendiz. Soy el narrador del Multiverso y te guiaré en el primer turno.',
        target: '#tutorial'
    },
    {
    ...
    {
        text: 'Gira una tierra en el campo para generar maná del color del mismo y poder lanzar las cartas de criaturas o hechizos.',
    ...
    {
        text: 'Usa PASAR para terminar el turno y dejar que el rival tome su turno.',
        target: '#pasar'
    },
    {
...
function jugar(carta) {
    ...
    if (turn !== 'player') {
        log(`Solo puedes jugar cartas en el turno de ${playerName}.`);
        return;
    }
...
document.getElementById("NoBloqueo").addEventListener("click", async function () {
    ...
    // Procesar solo el atacante actual sin bloqueadores
    const atacante = atacantesNoBloqueados.shift();
    ...
    const siguiente = atacantesNoBloqueados[0];
    if (siguiente) {
        log(`Atacante siguiente: ${siguiente.name}. Selecciona bloqueadores o haz clic en "No bloquear".`);
    }
});
...
function enderezarCriaturas(turnoActual) {
    ...
    battlefield.forEach(carta => {
        ...
        if (carta.element) {
            ...
            delete carta.element.dataset.mana;
...
async function enemyTurn() {
    ...
    const primerAtacante = atacantesNoBloqueados[0];
    if (primerAtacante) {
        log(`⚔️ Atacante actual: ${primerAtacante.name} (${primerAtacante.power} poder). Selecciona bloqueadores y confirma, o haz clic en "No bloquear".`);
    }
}   
updateManaUI();
updateLifeUI();
...
function updateLifeUI() {
    const playerFill = document.getElementById("playerLifeFill");
    const enemyFill = document.getElementById("enemyLifeFill");
    const playerText = document.getElementById("playerLife");
    const enemyText = document.getElementById("enemyLife");

    if (playerText) playerText.textContent = playerLife;
    if (enemyText) enemyText.textContent = enemyLife;
    if (playerFill) playerFill.style.width = `${Math.max(0, Math.min(100, (playerLife / PLAYER_MAX_LIFE) * 100))}%`;
    if (enemyFill) enemyFill.style.width = `${Math.max(0, Math.min(100, (enemyLife / ENEMY_MAX_LIFE) * 100))}%`;
}
...
//-----------------------MANA---------------------------------
function manaPieceHtml(symbol, count) {
    return `<span class="mana-piece mana-${symbol.toLowerCase()}"><span class="mana-symbol">${symbol}</span><span class="mana-value">${count}</span></span>`;
}

function manaPoolToHtml(pool) {
    return [
        manaPieceHtml('W', pool.W),
        manaPieceHtml('U', pool.U),
        manaPieceHtml('B', pool.B),
        manaPieceHtml('R', pool.R),
        manaPieceHtml('G', pool.G),
        manaPieceHtml('C', pool.C),
    ].join(' ');
}
...
function updateManaUI() {
    ...
    if (playerMana) playerMana.innerHTML = manaPoolToHtml(playerManaPool);
    if (enemyMana) enemyMana.innerHTML = manaPoolToHtml(enemyManaPool);
}
...
function tapLandForMana(carta, turnoActual) {
    ...
    tapCarta(carta);
    if (carta.element) {
        carta.element.dataset.mana = manaColor;
    }
