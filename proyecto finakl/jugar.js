...
function enemyTurn() {
    ...
    tierrasEnemy.forEach(tierra => tapLandForMana(tierra, 'enemy'));
    ...
    if (manoRival.length > 0) {
        ...
        const hechizoJugable = manoRival.find(c => c.type !== 'land' && canPayCost(c.cost, enemyManaPool));
        if (hechizoJugable) {
    ...
    const atacantesEnemy = EnemyBattlefield.filter(c => c.type === 'creature' && !c.tapped && !c.mareoInvocacion);
